...
// update
if( !empty($_GET['id']) && !empty($_GET['tableName']) && $_GET['role']=='user'){
   $id=legal_input($_GET['id']);
   $tableName=legal_input($_GET['tableName']);
   $query="SELECT status FROM user_profile WHERE id=$id";
   $res= $db->query($query);
   $role=$res->fetch_assoc();
   if($role['status']==1)
   {
...
// insert
if(empty($_GET['id']) && !empty($_GET['name']) && $_GET['name']=='user_profile'){
   extract($_POST);
  if(!empty($full_name) & !empty($password)){
   if($password==$cpassword)
   {
    ...
    if(!empty($data) && !empty($tableName)){
       $insertData=insert_data($data,$tableName);
       if($insertData){
         echo "<span class='success'>User Profile saved sucessfully</span>";
       }else{
         echo "<span class='fail'>Error!.. check your query</span>";
       }
    }
  ...
  echo "<span class='fail'>User Profile field is empty</span>";
}
